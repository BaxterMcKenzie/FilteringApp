...
// Get the input elements - our filters
...
// Get out sorting buttons
const highToLowBtn = document.getElementById("price-high-to-low");
const lowToHighBtn = document.getElementById("price-low-to-high");
const alphabeticalBtn = document.getElementById("alphabetical-button");
...
alphabeticalBtn.addEventListener('click', function () {
    const filteredProperties = filterProperties();
    const sortedProperties = sortPropertiesByAlphabetical(filteredProperties); // sorting filtered properties by name alphabetically
    populateResults(sortedProperties);
});

// ******** ---------- FILTERING FUNCTION & LOGIC ----------- ***********

// Function to filter results - return an array of filtered properties
...
function sortPropertiesByAlphabetical(properties) {
    return properties.sort((a, b) => {
        const nameA = a.name.toLowerCase(); // property a name to lower case
        const nameB = b.name.toLowerCase(); // property b name to lower case
...
function attachModalToImages() {
    ...
    for (let i = 0; i < images.length; i++) {
        images[i].addEventListener('click', function (event) {
            ...
            const imageTop = rect.top + scrollPosition; // Calculating the top position for the clicked image
            const windowHeight = window.innerHeight;
            const dialogHeight = detailsModal.offsetHeight; // height of the modal
            const viewportTop = scrollPosition;

            let dialogTop = viewportTop + (windowHeight - dialogHeight) / 2; // Calculate the top position for the dialog to be centred 

            // Ensure the dialog doesnt go above or below the veiwport:
            if (dialogTop < imageTop) {
                ...
                dialogTop = windowHeight + viewportTop - dialogHeight; // Place dialog at the bottom of the viewport if theres not enough space
            }
            ...
            document.body.classList.add('modal-open'); // add class to diable scrolling
